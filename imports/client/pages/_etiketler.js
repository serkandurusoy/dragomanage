...
import { EtiketModal as modal } from '/imports/client/modals';
...
export default createContainer(props => {
  ...
  const cursor = Etiketler.find(selector, {sort: {tip: 1, isim: 1}, limit: limit.get()});
...
}, class Etiketler extends Component {
  ...
  render() {
    ...
    return <Container yetki={this.props.route.yetki}>
      <PageHeader title="Etiketler" toggleSearchPane={this.toggleSearchPane} modalOpen={this.modalOpen}/>
      {
        ...
        <Row className="searchRow">
          ...
          >
            <Col sm={6} className="searchContainer">
              <TextField autofocus name="keyword" label={false} placeholder="Etiket" />
            </Col>
            <Col sm={6} className="searchContainer">
              <SelectField
                label={false}
            ...
            <tbody>
              {
                records.length === 0
                  ...
                  : records.map(record =>
                      <tr key={record._id}>
                        <td data-th="Tip" className="noWrap">{record.tip.enumValueToLabel(ETIKETLER)}</td>
