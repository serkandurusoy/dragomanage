...
export default createContainer(props => {
  ...
  const subscriptionsReady = [
    Meteor.subscribe('cariAcilisBakiyeler', {selectorOptions: selectorOptions.get(), limit: limit.get()}),
...
}, class CariAcilisBakiyeler extends Component {
  ...
  render() {
    ...
    return <Container yetki={this.props.route.yetki}>
      <PageHeader title="Cari açılış bakiyeler" toggleSearchPane={this.toggleSearchPane} hideAddButton={true} />
      {
        ...
        <Row className="searchRow">
          ...
          >
            <Col sm={4} className="searchContainer">
              <TextField name="keyword" label={false} placeholder="İsim veya ünvan" />
            </Col>
            <Col sm={4} className="searchContainer">
              <SelectField
                label={false}
            ...
            <tbody>
              {
                ...
                : records.map(record =>
                    <tr key={record._id}>
                      ...
                      <td data-th="Bakiye" className="text-right noWrap">{record.bakiyeAcilisLabelTL()}</td>
                    </tr>
                  )
              }
              {
