...
export default withTracker(props => {
...
})(class KasaAcilisBakiyeleri extends Component {
  ...
  render() {
    ...
    return <Container yetki={this.props.route.yetki}>
      <PageHeader title="Kasa açılış bakiyeleri" toggleSearchPane={this.toggleSearchPane} hideAddButton={true}/>
      {
        ...
        <Row className="searchRow">
          ...
          >
            <Col sm={4} className="searchContainer">
              <TextField autofocus name="keyword" label={false} placeholder="İsim" />
            </Col>
            <Col sm={4} className="searchContainer">
              <SelectField
                label={false}
        ...
        <Col xs={12}>
          <Table className="customResponsive" striped hover>
            ...
            <tr>
              ...
              <th>Kur</th>
              <th>İsim</th>
              <th className="text-right">Açılış bakiyesi</th>
            </tr>
            </thead>
            <tbody>
              {
                records.length === 0
                  ...
                  : records.map(record =>
                      <tr key={record._id}>
                        ...
                        <td data-th="Açılış bakiyesi" className="text-right noWrap">{record.kur !== KURLAR.TRY.value ? `(${record.bakiyeAcilisLabel()}) ` : ''}{record.bakiyeAcilisLabelTL()}</td>
                      </tr>
                    )
              }
              {
