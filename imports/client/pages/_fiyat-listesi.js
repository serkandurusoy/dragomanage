...
import { ETIKETLER, URUNLER, SINIFLAR, KURLAR } from '/imports/environment/enums';
...
export default createContainer(props => {
  ...
  render() {
    ...
    return <Container yetki={this.props.route.yetki}>
      <PageHeader title="Fiyat listesi" toggleSearchPane={this.toggleSearchPane} hideAddButton={true} />
      {
        ...
        <Col xs={12}>
          <Table className="customResponsive" striped hover>
            ...
            <tbody>
              {
                records.length === 0
                ? (!subscriptionsLoading && <TableRowEmpty colSpan={3}/>)
                : records.map(record =>
                    <tr key={record._id}>
                      <td data-th="İsim">{record.label()}</td>
                      <td data-th="Fiyat" className="text-right noWrap">{record.kur !== KURLAR.TRY.value ? `(${record.fiyatLabel()}) ` : ''}{record.fiyatLabelTL()}</td>
                      <td data-th="Özel fiyat" className="text-right noWrap">{record.kur !== KURLAR.TRY.value ? `(${record.ozelFiyatLabel()}) ` : ''}{record.ozelFiyatLabelTL()}</td>
                    </tr>
                  )
              }
              {
                subscriptionsLoading && <TableRowLoading colSpan={3}/>
              }
            </tbody>
          </Table>
        </Col>
