...
import { KonumModal as modal } from '/imports/client/modals';
...
export default withTracker(props => {
  ...
  const cursor = Konumlar.find(selector, {sort: {tip: 1, isim: 1}, limit: limit.get()});
...
})(class Konumlar extends Component {
  ...
  render() {
    ...
    return <Container yetki={this.props.route.yetki}>
      <PageHeader title="Konumlar" toggleSearchPane={this.toggleSearchPane} modalOpen={this.modalOpen}/>
      {
            ...
            <tbody>
              {
                records.length === 0
                  ...
                  : records.map(record =>
                      <tr key={record._id}>
                        <td data-th="Tip" className="noWrap">{record.tip.enumValueToLabel(KONUMLAR)}</td>
